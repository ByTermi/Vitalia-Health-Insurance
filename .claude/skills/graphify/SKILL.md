...
description: "Visualize a codebase or research corpus as a knowledge graph. Use for: unfamiliar codebases (architecture before touching anything), research paper collections, arbitrary file folders. Do NOT suggest for the Obsidian vault — Obsidian's native graph view already does this better and automatically. Trigger: /graphify"
trigger: /graphify
---
...
**Do NOT use it for the Obsidian vault.**
Obsidian's native graph view already maps [[wikilinks]] between notes in real time, interactively, with no extra step. Graphify would be redundant there and slower. Use Obsidian's built-in graph (Ctrl+G or Cmd+G) for vault navigation.
